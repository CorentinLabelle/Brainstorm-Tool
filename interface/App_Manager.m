    ...
    methods
        ...
        function selectRawStudies(obj)
            ...
            for i = 1:length(subjects)
                ...
                for j = 1:length(studies)
                    if contains(studies(j).Text, '@')

                        obj.App.Tree.CheckedNodes = [obj.App.Tree.CheckedNodes' studies(j)];
        ...
        function [folder, fileNames, extensionOut] = get_File_Path(obj, title, extensionIn, startingFolder)
            ...
            % File Manager                    
            [fileNames, folder] = uigetfile(extensionIn', title, startingFolder, ...
                'MultiSelect', 'on');
            ...
            extensionOut = cell(1, length(fileNames));
            for i = 1:length(fileNames)
                fileNameSplit = split(fileNames(i), '.');
        ...
        function create_History_Event_Button(obj, sFiles, type)
            ...
            folder = obj.App.controller.getWorkingFolderPath;
...
%                     obj.FontColor = textColor;
        ...
        function [rawFilesPath, extension] = review_Raw_Files(obj, title, extension_in)
            ...
            rawFilesPath = fullfile(folder, rawFileNames);
            
            obj.App.controller.asgRawDataSearchPath(folder);
