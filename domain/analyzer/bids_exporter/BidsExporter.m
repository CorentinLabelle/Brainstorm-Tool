classdef BidsExporter < handle
    ...
    methods (Access = public)
        ...
        function sFiles = export(obj, listOfParameters, sFiles)            
            ...
            eventDescriptorPath = listOfParameters.getConvertedValue(2);
            
            for i = 1:length(sFiles)
                disp(['EXPORTING TO BIDS> ' num2str(i) ' of ' num2str(length(sFiles)) ' files.']);
    ...
    methods (Static, Access = private)  
        ...
        function sFile = brainstormExportToBidsFunction(listOfParameters, sFile, bidsPath)
            taskDescription = listOfParameters.getConvertedValue(8);
            datasetDesc = BidsExporter.formatStructToCharacters(listOfParameters.getConvertedValue(9));
            author = listOfParameters.getConvertedValue(5);
            sFile = bst_process('CallProcess', 'process_export_bids', sFile, [], ...
                 ...
                 'eegreference',  'Cz', ...
                 'edit',          struct(...
                     'ProjName',    listOfParameters.getConvertedValue(3), ...
                     'ProjID',      listOfParameters.getConvertedValue(4), ...
                     'ProjDesc',    listOfParameters.getConvertedValue(6), ...
                     'Categories',  listOfParameters.getConvertedValue(7), ...
                     'JsonDataset', ['{' 10 '  "License": "PD"' datasetDesc '}'], ...
                     'JsonMeg',     ['{' 10 '  "TaskDescription": "' taskDescription '"}']));
            
             if isempty(sFiles)
                error('Error in Brainstorm''s Export to BIDS function. Output is empty.');
             end
        end
